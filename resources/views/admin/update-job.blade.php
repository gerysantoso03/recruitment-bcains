            ...
            <form action="{{ route('add.job') }}" method="post" class="border-2 border-sky-900 rounded-lg flex flex-col gap-4 p-4">
                ...
                <div class="flex justify-start items-center gap-8">
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                        ...
                        <input type="text" value="{{ $job['position_name'] }}" class="border border-sky-900 rounded-lg p-3" name="position_name">
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                        ...
                        <select class="border border-sky-900 rounded-lg p-3" name="department_id">
                            @foreach ($departments as $department)
                                <option {{ $job->department['id'] == $department['id'] ? 'selected' : '' }} value="{{ $department['id'] }}">{{ $department['department_name'] }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                        ...
                        <select class="border border-sky-900 rounded-lg p-3" name="branch_id">
                            @foreach ($branchs as $branch)
                                <option {{ $job->branch['id'] == $branch['id'] ? 'selected' : '' }} value="{{ $branch['id'] }}">{{ $branch['branch_name'] }}</option>
                            @endforeach
                        </select>
                ...
                <div class="flex justify-start items-center gap-8">
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                        ...
                        <input type="date" value="{{ $job['end_date'] }}" class="border border-sky-900 rounded-lg p-3" name="end_date">
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                        ...
                        <select class="border border-sky-900 rounded-lg p-3" name="employment_type">
                            ...
                            <option {{ $job['employment_type'] == 'Contract' ? 'selected' : '' }} value="Contract">Contract</option>
                            <option {{ $job['employment_type'] == 'Outsource' ? 'selected' : '' }}  value="Outsource">Outsource</option>
                            <option {{ $job['employment_type'] == 'Internship' ? 'selected' : '' }} value="Internship">Internship</option>
                            <option {{ $job['employment_type'] == 'BDP' ? 'selected' : '' }} value="BDP">BDP</option>
                        </select>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <textarea class="border border-sky-900 rounded-lg p-3" name="qualifications" id="jobQualification" cols="30" rows="10">{{ $job['qualifications'] }}</textarea>
